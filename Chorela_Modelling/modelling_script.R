...
cat("\nDone. Tables written to working directory:\n",
    " - cfr_by_year.csv, cfr_by_county.csv, mortality_odds_ratios.csv, case_forecast_12mo.csv\n",
    "(Plots are printed to the active graphics device, not saved — see section headers above.)\n")

kable(
  cfr_by_year,
  caption = "Cholera Case Fatality Rate by Year",
  digits = 2,
  format = "html"
) %>%
...
kable(
  cfr_by_county,
  caption = "Cholera Case Fatality Rate by County",
  digits = 2,
  format = "html"
) %>%
  kable_styling(full_width = FALSE)
...
kable(
  or_table,
  caption = "Odds Ratios for Cholera Mortality",
  digits = 2,
  format = "html"
) %>%
  kable_styling(full_width = FALSE)
...
kable(
  as_tibble(fc),
  caption = "12-Month Cholera Case Forecast",
  digits = 2,
  format = "html"
) %>%
  kable_styling(full_width = FALSE)
